...
one_week_clean <- read.csv(here("analysis_code", 'one_week_clean_gpsplot.csv')) %>%
  one_week_clean <- read.csv(here("analysis_code", 'one_week_clean_gpsplot.csv')) %>%
  mutate(day_of_week = factor(one_week_clean$day_of_week, levels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"))) %>%
  mutate(time_color = factor(one_week_clean$time_color, levels = c("Morning", "Afternoon", "Evening")))
...
create_comprehensive_dbscan_plot <- function(seed) {
  ...
  home_points <- data.frame(
    lon = rnorm(40, 0.0, 0.0007),
    lat = rnorm(40, 0.0, 0.0007),
    type = "Stay Location",
    stay_type = "Home",  # Keep original stay_type
    cluster_id = "Cluster"
  ) %>%
  rowwise() %>%
  mutate(distance_to_centre = distHaversine(c(lon, lat), c(0.0, 0.0))) %>%
  ungroup() %>%
  mutate(
  ...
  work_points <- data.frame(
    lon = rnorm(30, 0.015, 0.0007),
    lat = rnorm(30, 0.010, 0.0007), 
    type = "Stay Location",
    stay_type = "Work",  # Keep original stay_type
    cluster_id = "Cluster"
  ) %>%
  rowwise() %>%
  mutate(distance_to_centre = distHaversine(c(lon, lat), c(0.015, 0.010))) %>%
  ungroup() %>%
  mutate(
  ...
  shop_points <- data.frame(
    lon = rnorm(22, 0.005, 0.0005),
    lat = rnorm(22, -0.005, 0.0005),
    type = "Stay Location", 
    stay_type = "Shop",  # Keep original stay_type
    cluster_id = "Cluster"
  ) %>%
  rowwise() %>%
  mutate(distance_to_centre = distHaversine(c(lon, lat), c(0.005, -0.005))) %>%
  ungroup() %>%
  mutate(
...
movement_points <- data.frame(
  lon = c(0.002, 0.004, 0.006, 0.008, 0.012,  # Path HOME to WORK
          0.001, 0.003,                        # Path HOME to SHOP
          0.011, 0.007),                       # Path WORK to SHOP
  lat = c(0.001, 0.002, 0.003, 0.005, 0.008,  # Path HOME to WORK
          -0.002, -0.003,                      # Path HOME to SHOP
          0.007, 0.000),                       # Path WORK to SHOP
  type = "Movement",
  ...
  path_home_work <- data.frame(
    lon = c(0.0, 0.002, 0.004, 0.006, 0.008, 0.012, 0.015),
    lat = c(0.0, 0.001, 0.002, 0.003, 0.005, 0.008, 0.010),
    path_id = "home_work"
  )
  
  path_home_shop <- data.frame(
    lon = c(0.0, 0.001, 0.003, 0.005),
    lat = c(0.0, -0.002, -0.003, -0.005),
    path_id = "home_shop"
  )
  
  path_work_shop <- data.frame(
    lon = c(0.015, 0.011, 0.007, 0.005),
    lat = c(0.010, 0.007, 0.000, -0.005),
    path_id = "work_shop"
  )
  
  # Create DBSCAN radius circles
  radius <- 0.0015
  home_circle <- create_circle(0.0, 0.0, radius)
  work_circle <- create_circle(0.015, 0.010, radius) 
  shop_circle <- create_circle(0.005, -0.005, radius)
  ...
  ggplot() +
    ...
    # Add location labels
    annotate("text", x = 0.0, y = -0.0025, label = "HOME", 
             fontface = "bold", size = 6, hjust = 0.5, color = "#C73E1D") +
    annotate("text", x = 0.015, y = 0.0125, label = "WORK", 
             fontface = "bold", size = 6, hjust = 0.5, color = "#2E86AB") +
    annotate("text", x = 0.005, y = -0.0075, label = "SHOP", 
             fontface = "bold", size = 6, hjust = 0.5, color = "#F18F01") +
    
    # Add these annotations for a clean manual legend (bigger, moved down and right)
annotate("point", x = 0.023 - 0.007, y = -0.001, size = 4, color = "black") +
annotate("text", x = 0.025 - 0.007, y = -0.001, label = "Stationary Behavior", hjust = 0, size = 4) +

annotate("point", x = 0.023 - 0.007, y = -0.003, shape = 4, size = 4, color = "black") +
annotate("text", x = 0.025 - 0.007, y = -0.003, label = "Isolated GPS Points", hjust = 0, size = 4) +

annotate("segment", x = 0.023 - 0.008, xend = 0.024 - 0.007, y = -0.005, yend = -0.005, 
         linetype = "dotted", size = 1, color = "black") +
annotate("text", x = 0.025 - 0.007, y = -0.005, label = "Travel Between Stays", hjust = 0, size = 4) +
